...
//  source: /Users/retechretech/dev/tools/lib/j2objc/jre_emul/apache_harmony/classlib/modules/luni/src/main/java/java/io/File.java
//
...
- (NSString *)properPath_ {
  return [[properPath__ retain] autorelease];
}
...
+ (void)setCounterWithInt:(int)counter {
  JavaIoFile_counter_ = counter;
}
...
+ (void)setCounterBaseWithInt:(int)counterBase {
  JavaIoFile_counterBase_ = counterBase;
}
...
+ (void)setTempFileLockerWithJavaIoFile_TempFileLocker:(JavaIoFile_TempFileLocker *)tempFileLocker {
...
+ (void)setCaseSensitiveWithBOOL:(BOOL)caseSensitive {
  JavaIoFile_caseSensitive_ = caseSensitive;
}
...
- (BOOL)canRead {
  ...
  return [self existsImplWithNSString:pp] && ![self isWriteOnlyImplWithNSString:pp];
}
...
- (BOOL)canWrite {
  ...
  return exists && ![self isReadOnlyImplWithNSString:[self properPathWithBOOL:YES]];
}
...
- (BOOL)delete__ {
  ...
  if (([NIL_CHK(path_) length] != 0) && [self isDirectoryImplWithNSString:propPath]) {
...
- (BOOL)isReadOnlyImplWithNSString:(NSString *)filePath   {
    return [[NSFileManager defaultManager] isReadableFileAtPath:filePath];
  }

- (BOOL)isWriteOnlyImplWithNSString:(NSString *)filePath   {
...
- (IOSObjectArray *)list {
  ...
  if (![self isDirectoryImplWithNSString:bs] || ![self existsImplWithNSString:bs] || [self isWriteOnlyImplWithNSString:bs]) {
    return nil;
  }
  ...
  IOSObjectArray *result = [[[IOSObjectArray alloc] initWithLength:(int) [((IOSObjectArray *) NIL_CHK(implList)) count] type:[IOSClass classWithClass:[NSString class]]] autorelease];
  for (int index = 0; index < (int) [((IOSObjectArray *) NIL_CHK(implList)) count]; index++) {
    [((IOSObjectArray *) NIL_CHK(result)) replaceObjectAtIndex:index withObject:[NSString stringWithString:((NSString *) [((IOSObjectArray *) NIL_CHK(implList)) objectAtIndex:index])]];
  }
  return result;
}
