  ...
  for (int i = 0; i < length; i++) {
    ...
    if (pathChar == '/') {
      if ((foundSlash && i == uncIndex) || !foundSlash) {
        (*[newPath charRefAtIndex:newLength++]) = JavaIoFile_separatorChar;
        foundSlash = YES;
      }
    }
    else {
      if (pathChar == ':' && uncIndex > 0 && (newLength == 2 || (newLength == 3 && [newPath charAtIndex:1] == JavaIoFile_separatorChar)) && [newPath charAtIndex:0] == JavaIoFile_separatorChar) {
        (*[newPath charRefAtIndex:0]) = [newPath charAtIndex:newLength - 1];
        newLength = 1;
        uncIndex = 2;
      }
      (*[newPath charRefAtIndex:newLength++]) = pathChar;
      foundSlash = NO;
    }
  }
...
- (IOSByteArray *)resolveWithByteArray:(IOSByteArray *)newResult {
  ...
  for (int i = 1; i <= (int) [((IOSByteArray *) nil_chk(newResult)) count]; i++) {
    if (i == (int) [newResult count] || [newResult byteAtIndex:i] == JavaIoFile_separatorChar) {
      ...
      if (linkPath == newResult) {
        ...
        if (!done) {
          inPlace = YES;
          (*[newResult byteRefAtIndex:i]) = 0x0000;
        }
      }
      else {
        nextSize = i - last + 1;
        linkSize = (int) [((IOSByteArray *) nil_chk(linkPath)) count];
        if ([linkPath byteAtIndex:linkSize - 1] == JavaIoFile_separatorChar) {
          linkSize--;
        }
      ...
      linkPath = [self resolveLinkWithByteArray:bytes withInt:inPlace ? i : (int) [((IOSByteArray *) nil_chk(bytes)) count] withBOOL:YES];
      if (inPlace) {
        (*[newResult byteRefAtIndex:i]) = '/';
      }
      last = i + 1;
    }
  }
...
+ (IOSObjectArray *)__exceptions_resolveLinkWithByteArray_withInt_withBOOL_ {
  return [IOSObjectArray arrayWithObjects:(id[]) { [JavaIoIOException getClass] } count:1 type:[IOSClass getClass]];
}
...
- (IOSObjectArray *)listFiles {
  ...
  for (int i = 0; i < resultLength; i++) {
    [results replaceObjectAtIndex:i withObject:[[[JavaIoFile alloc] initWithJavaIoFile:self withNSString:[tempNames objectAtIndex:i]] autorelease]];
  }
  return results;
}
...
- (IOSObjectArray *)listFilesWithJavaIoFileFilter:(id<JavaIoFileFilter>)filter {
  ...
  for (int index = 0; index < (int) [((IOSObjectArray *) nil_chk(implList)) count]; index++) {
    NSString *aName = [NSString stringWithString:[implList objectAtIndex:index]];
...
- (NSString *)getAbsoluteName {
  ...
  if ([f isDirectory] && [name charAtWithInt:[((NSString *) nil_chk(name)) length] - 1] != JavaIoFile_separatorChar) {
    name = [((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) [[[JavaLangStringBuilder alloc] initWithInt:[name length] + 1] autorelease]) appendWithNSString:name])) appendWithUnichar:'/'])) description];
  }
  return name;
}
