...
- (void)unreadWithCharArray:(IOSCharArray *)buffer
  ...
  @synchronized (lock_) {
    ...
    for (int i = offset + length - 1; i >= offset; i--) {
      [self unreadWithInt:[buffer charAtIndex:i]];
    }
  }
}
...
- (void)unreadWithInt:(int)oneChar {
  @synchronized (lock_) {
    ...
    (*[((IOSCharArray *) nil_chk(buf_)) charRefAtIndex:--pos_]) = (unichar) oneChar;
  }
}
...
- (long long int)skipWithLongInt:(long long int)count {
  ...
  @synchronized (lock_) {
    ...
    if (availableFromBuffer > 0) {
      ...
      inSkipped = [((JavaIoReader *) nil_chk(in_)) skipWithLongInt:requiredFromIn];
    }
    else {
      inSkipped = [((JavaIoReader *) nil_chk(in_)) skipWithLongInt:count];
    }
