...
- (int)read {
  ...
  if (pos_ < (int) [((IOSByteArray *) nil_chk(buf_)) count]) {
    return ([buf_ byteAtIndex:pos_++] & (int) 0xFF);
  }
...
- (long long int)skipWithLongInt:(long long int)count {
  ...
  if (numSkipped < count) {
    numSkipped += [((JavaIoInputStream *) nil_chk(in_)) skipWithLongInt:count - numSkipped];
  }
  return numSkipped;
}
...
- (void)unreadWithInt:(int)oneByte {
  ...
  (*[((IOSByteArray *) nil_chk(buf_)) byteRefAtIndex:--pos_]) = (char) oneByte;
}
