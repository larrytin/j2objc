...
+ (NSString *)bigInteger2StringWithJavaMathBigInteger:(JavaMathBigInteger *)val
  ...
  if (radix != 16) {
    ...
    while (YES) {
      ...
      do {
        (*[result charRefAtIndex:--currentChar]) = [JavaLangCharacter forDigitWithInt:resDigit % radix withInt:radix];
      }
      while (((resDigit /= radix) != 0) && (currentChar != 0));
      int delta = charsPerInt - previous + currentChar;
      for (i = 0; i < delta && currentChar > 0; i++) {
        (*[result charRefAtIndex:--currentChar]) = '0';
      }
      for (i = tempLen - 1; (i > 0) && ([temp intAtIndex:i] == 0); i--) {
        ;
      }
      tempLen = i + 1;
      if ((tempLen == 1) && ([temp intAtIndex:0] == 0)) {
        break;
      }
    }
  }
    ...
    for (int i = 0; i < numberLength; i++) {
      for (int j = 0; (j < 8) && (currentChar > 0); j++) {
        ...
        (*[result charRefAtIndex:--currentChar]) = [JavaLangCharacter forDigitWithInt:resDigit withInt:16];
      }
    }
  }
  while ([result charAtIndex:currentChar] == '0') {
    currentChar++;
  }
  if (sign == -1) {
    (*[result charRefAtIndex:--currentChar]) = '-';
  }
...
+ (NSString *)toDecimalScaledStringWithJavaMathBigInteger:(JavaMathBigInteger *)val
  ...
  if (numberLength == 1) {
    ...
    if (highDigit < 0) {
      ...
      do {
        ...
        v /= 10;
        (*[result charRefAtIndex:--currentChar]) = (unichar) ((int) 0x0030 + ((int) (prev - v * 10)));
      }
      while (v != 0);
    }
    else {
      ...
      do {
        int prev = v;
        v /= 10;
        (*[result charRefAtIndex:--currentChar]) = (unichar) ((int) 0x0030 + (prev - v * 10));
      }
      while (v != 0);
    }
  }
    ...
    while (YES) {
      ...
      for (int i1 = tempLen - 1; i1 >= 0; i1--) {
        long long int temp1 = (result11 << 32) + ([temp intAtIndex:i1] & (long long) 0xFFFFFFFFLL);
        long long int res = [JavaMathConversion divideLongByBillionWithLongInt:temp1];
        (*[temp intRefAtIndex:i1]) = (int) res;
        result11 = (int) (res >> 32);
      }
      int resDigit = (int) result11;
      int previous = currentChar;
      do {
        (*[result charRefAtIndex:--currentChar]) = (unichar) ((int) 0x0030 + (resDigit % 10));
      }
  ...
  if ((scale_ > 0) && (exponent >= -6)) {
    if (exponent >= 0) {
      ...
      for (int j = resLengthInChars - 1; j >= insertPoint; j--) {
        (*[result charRefAtIndex:j + 1]) = [result charAtIndex:j];
      }
      (*[result charRefAtIndex:++insertPoint]) = '.';
      if (negNumber) {
        (*[result charRefAtIndex:--currentChar]) = '-';
      }
  ...
  if (negNumber) {
    [result1 appendWithUnichar:'-'];
  }
  if (endPoint - startPoint >= 1) {
    [result1 appendWithUnichar:[result charAtIndex:currentChar]];
    [result1 appendWithUnichar:'.'];
...
+ (NSString *)toDecimalScaledStringWithLongInt:(long long int)value
  ...
  [result1 appendWithNSString:[JavaLangLong toStringWithLongInt:exponent]];
  return [result1 description];
}
...
+ (double)bigInteger2DoubleWithJavaMathBigInteger:(JavaMathBigInteger *)val {
  if ((((JavaMathBigInteger *) nil_chk(val))->numberLength_ < 2) || ((val->numberLength_ == 2) && ([((IOSIntArray *) nil_chk(val->digits_)) intAtIndex:1] > 0))) {
  ...
  int delta = bitLen - 54;
  long long int lVal = [((JavaMathBigInteger *) nil_chk([((JavaMathBigInteger *) nil_chk([val abs])) shiftRightWithInt:delta])) longValue];
