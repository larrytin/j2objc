...
+ (JavaMathBigInteger *)addWithJavaMathBigInteger:(JavaMathBigInteger *)op1
  ...
  if (op1Len + op2Len == 2) {
    long long int a = ([((IOSIntArray *) nil_chk(op1->digits_)) intAtIndex:0] & (long long) 0xFFFFFFFFLL);
    long long int b = ([op2->digits_ intAtIndex:0] & (long long) 0xFFFFFFFFLL);
    long long int res;
    int valueLo;
    int valueHi;
    if (op1Sign == op2Sign) {
...
+ (void)addWithIntArray:(IOSIntArray *)res
  ...
  (*[((IOSIntArray *) nil_chk(res)) intRefAtIndex:0]) = (int) carry;
  carry >>= 32;
  if (aSize >= bSize) {
    for (i = 1; i < bSize; i++) {
      carry += ([a intAtIndex:i] & (long long) 0xFFFFFFFFLL) + ([b intAtIndex:i] & (long long) 0xFFFFFFFFLL);
      (*[res intRefAtIndex:i]) = (int) carry;
      carry >>= 32;
    }
    for (; i < aSize; i++) {
      carry += [a intAtIndex:i] & (long long) 0xFFFFFFFFLL;
      (*[res intRefAtIndex:i]) = (int) carry;
      carry >>= 32;
    }
  }
    ...
    for (; i < bSize; i++) {
      carry += [b intAtIndex:i] & (long long) 0xFFFFFFFFLL;
      (*[res intRefAtIndex:i]) = (int) carry;
      carry >>= 32;
    }
  }
...
+ (void)subtractWithIntArray:(IOSIntArray *)res
  ...
  for (i = 0; i < bSize; i++) {
    ...
    (*[((IOSIntArray *) nil_chk(res)) intRefAtIndex:i]) = (int) borrow;
    borrow >>= 32;
  }
  for (; i < aSize; i++) {
...
+ (void)inplaceAddWithJavaMathBigInteger:(JavaMathBigInteger *)op1
  ...
  if (carry == 1) {
    (*[((IOSIntArray *) nil_chk(op1->digits_)) intRefAtIndex:op1->numberLength_]) = 1;
    op1->numberLength_++;
  }
  [op1 unCache];
}
