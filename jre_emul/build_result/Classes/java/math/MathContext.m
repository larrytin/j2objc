...
- (id)initWithNSString:(NSString *)val {
  if ((self = [super init])) {
    ...
    for (i = 0; (i < (int) [((IOSCharArray *) nil_chk(JavaMathMathContext_chPrecision_)) count]) && ([charVal charAtIndex:i] == [JavaMathMathContext_chPrecision_ charAtIndex:i]); i++) {
      ;
    }
    if (i < (int) [JavaMathMathContext_chPrecision_ count]) {
      @throw [[[JavaLangIllegalArgumentException alloc] initWithNSString:@"bad string format"] autorelease];
    }
    digit = [JavaLangCharacter digitWithUnichar:[charVal charAtIndex:i] withInt:10];
    if (digit == -1) {
      @throw [[[JavaLangIllegalArgumentException alloc] initWithNSString:@"bad string format"] autorelease];
    }
    ...
    while (YES);
    for (j = 0; (j < (int) [((IOSCharArray *) nil_chk(JavaMathMathContext_chRoundingMode_)) count]) && ([charVal charAtIndex:i] == [JavaMathMathContext_chRoundingMode_ charAtIndex:j]); i++, j++) {
      ;
    }
