...
- (BOOL)addWithId:(JavaLangEnum *)element {
  ...
  if (oldBits != newBits) {
    (*[bits_ longRefAtIndex:index]) = newBits;
    size__++;
    return YES;
  }
  return NO;
...
- (BOOL)addAllWithJavaUtilCollection:(id<JavaUtilCollection>)collection {
  ...
  if ([(id) collection isKindOfClass:[JavaUtilEnumSet class]]) {
    ...
    for (int i = 0; i < (int) [((IOSLongArray *) nil_chk(bits_)) count]; i++) {
      long long int oldBits = [bits_ longAtIndex:i];
      long long int newBits = oldBits | [hugeSet->bits_ longAtIndex:i];
      if (oldBits != newBits) {
        (*[bits_ longRefAtIndex:i]) = newBits;
...
- (void)clear {
  [JavaUtilArrays fillWithLongArray:bits_ withLongInt:0];
  size__ = 0;
}
...
- (void)complement {
  ...
  for (int i = 0, length = (int) [((IOSLongArray *) nil_chk(bits_)) count]; i < length; i++) {
    long long int b = ~[bits_ longAtIndex:i];
    if (i == length - 1) {
...
- (BOOL)containsAllWithJavaUtilCollection:(id<JavaUtilCollection>)collection {
  ...
  if ([(id) collection isKindOfClass:[JavaUtilHugeEnumSet class]]) {
    ...
    if ([self isValidTypeWithIOSClass:set->elementClass_]) {
      for (int i = 0; i < (int) [((IOSLongArray *) nil_chk(bits_)) count]; i++) {
        long long int setBits = [((IOSLongArray *) nil_chk(set->bits_)) longAtIndex:i];
        if (([bits_ longAtIndex:i] & setBits) != setBits) {
          return NO;
        }
      }
      return YES;
...
- (BOOL)isEqual:(id)object {
  ...
  return [JavaUtilArrays equalsWithLongArray:bits_ withLongArray:((JavaUtilHugeEnumSet *) object)->bits_];
}
...
- (void)setRangeWithId:(JavaLangEnum *)start
  ...
  if (startIndex == endIndex) {
    ...
    (*[bits_ longRefAtIndex:startIndex]) |= range;
    size__ += [JavaLangLong bitCountWithLongInt:[bits_ longAtIndex:startIndex]];
  }
  else {
    ...
    size__ -= [JavaLangLong bitCountWithLongInt:[bits_ longAtIndex:endIndex]];
    (*[bits_ longRefAtIndex:endIndex]) |= range;
    size__ += [JavaLangLong bitCountWithLongInt:[bits_ longAtIndex:endIndex]];
    for (int i = (startIndex + 1); i <= (endIndex - 1); i++) {
      size__ -= [JavaLangLong bitCountWithLongInt:[bits_ longAtIndex:i]];
      (*[bits_ longRefAtIndex:i]) = -1LL;
      size__ += [JavaLangLong bitCountWithLongInt:[bits_ longAtIndex:i]];
    }
  }
}
...
- (id)initWithJavaUtilHugeEnumSet:(JavaUtilHugeEnumSet *)outer$ {
  if ((self = [super init])) {
    ...
    currentBits_ = [((IOSLongArray *) nil_chk(outer$->bits_)) longAtIndex:0];
...
- (void)computeNextElement {
  while (YES) {
    if (currentBits_ != 0) {
      ...
      currentBits_ = [this$0_->bits_ longAtIndex:index_];
    }
    else {
      mask_ = 0;
      return;
...
- (id)next {
  ...
  JavaUtilHugeEnumSet_HugeEnumSetIterator_set_last_(self, [((IOSObjectArray *) nil_chk(this$0_->enums_)) objectAtIndex:ordinal]);
  currentBits_ &= ~mask_;
  [self computeNextElement];
  return last_;
}
