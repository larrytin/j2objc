...
//  source: /Users/retechretech/dev/tools/lib/j2objc/jre_emul/android/libcore/luni/src/main/java/java/util/regex/Matcher.java
//
...
- (NSString *)input {
  return [[input_ retain] autorelease];
}
...
- (IOSIntArray *)matchOffsets {
  return [[matchOffsets_ retain] autorelease];
}
...
- (NSString *)groupWithInt:(int)group {
  [self ensureMatch];
  int from = [((IOSIntArray *) NIL_CHK(matchOffsets_)) intAtIndex:group * 2];
  int to = [((IOSIntArray *) NIL_CHK(matchOffsets_)) intAtIndex:(group * 2) + 1];
  if (from == -1 || to == -1) {
    return nil;
  }
  else {
