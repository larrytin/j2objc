...
- (id)initWithJavaUtilLinkedList:(JavaUtilLinkedList *)object
  ...
  if ((self = [super init])) {
    ...
    if (0 <= location && location <= list_->size__) {
      JavaUtilLinkedList_LinkIterator_set_link_(self, list_->voidLink_);
      if (location < list_->size__ / 2) {
        for (pos_ = -1; pos_ + 1 < location; pos_++) {
          JavaUtilLinkedList_LinkIterator_set_link_(self, ((JavaUtilLinkedList_Link *) nil_chk(link_))->next_);
        }
      }
      else {
        for (pos_ = list_->size__; pos_ >= location; pos_--) {
          JavaUtilLinkedList_LinkIterator_set_link_(self, ((JavaUtilLinkedList_Link *) nil_chk(link_))->previous_);
        }
      }
    }
    else {
...
- (void)addWithId:(id)object {
  if (expectedModCount_ == ((JavaUtilLinkedList *) nil_chk(list_))->modCount_) {
    ...
    JavaUtilLinkedList_LinkIterator_set_link_(self, newLink);
    JavaUtilLinkedList_LinkIterator_set_lastLink_(self, nil);
    pos_++;
    expectedModCount_++;
    list_->size__++;
    list_->modCount_++;
...
- (id)next {
  if (expectedModCount_ == ((JavaUtilLinkedList *) nil_chk(list_))->modCount_) {
    ...
    if (next != list_->voidLink_) {
      JavaUtilLinkedList_LinkIterator_set_lastLink_(self, JavaUtilLinkedList_LinkIterator_set_link_(self, next));
      pos_++;
...
- (id)previous {
  if (expectedModCount_ == ((JavaUtilLinkedList *) nil_chk(list_))->modCount_) {
    if (link_ != list_->voidLink_) {
      JavaUtilLinkedList_LinkIterator_set_lastLink_(self, link_);
      JavaUtilLinkedList_LinkIterator_set_link_(self, ((JavaUtilLinkedList_Link *) nil_chk(link_))->previous_);
      pos_--;
...
- (void)remove {
  if (expectedModCount_ == ((JavaUtilLinkedList *) nil_chk(list_))->modCount_) {
    if (lastLink_ != nil) {
      ...
      JavaUtilLinkedList_LinkIterator_set_link_(self, previous);
      JavaUtilLinkedList_LinkIterator_set_lastLink_(self, nil);
      expectedModCount_++;
      list_->size__--;
      list_->modCount_++;
...
- (void)dealloc {
  JreMemDebugRemove(self);
  JavaUtilLinkedList_LinkIterator_set_lastLink_(self, nil);
  JavaUtilLinkedList_LinkIterator_set_link_(self, nil);
...
- (void)copyAllFieldsTo:(JavaUtilLinkedList_LinkIterator *)other {
  ...
  JavaUtilLinkedList_LinkIterator_set_lastLink_(other, lastLink_);
  JavaUtilLinkedList_LinkIterator_set_link_(other, link_);
