...
- (id)initWithJavaUtilMap:(id<JavaUtilMap>)map {
  if (self = [super init]) {
    ...
    if ([(id) map isKindOfClass:[JavaUtilEnumMap class]]) {
      [self initializationWithJavaUtilEnumMap:(JavaUtilEnumMap *) check_class_cast(map, [JavaUtilEnumMap class])];
    }
    else {
...
- (void)clear {
  ...
  [JavaUtilArrays fillWithBooleanArray:hasMapping_ withBoolean:NO];
  mappingsCount_ = 0;
}

- (JavaUtilEnumMap *)clone {
  @try {
    JavaUtilEnumMap *enumMap = (JavaUtilEnumMap *) check_class_cast([super clone], [JavaUtilEnumMap class]);
...
- (BOOL)containsKeyWithId:(id)key {
  if ([self isValidKeyTypeWithId:key]) {
    ...
    return IOSBooleanArray_Get(nil_chk(hasMapping_), keyOrdinal);
  }
  return NO;
}
...
- (BOOL)containsValueWithId:(id)value {
  if (nil == value) {
    for (int i = 0; i < enumSize_; i++) {
      if (IOSBooleanArray_Get(nil_chk(hasMapping_), i) && nil == IOSObjectArray_Get(nil_chk(values__), i)) {
        return YES;
      }
    }
  }
    ...
    for (int i = 0; i < enumSize_; i++) {
      if (IOSBooleanArray_Get(nil_chk(hasMapping_), i) && [value isEqual:IOSObjectArray_Get(nil_chk(values__), i)]) {
        return YES;
      }
    }
  }
...
- (BOOL)isEqual:(id)object {
  ...
  JavaUtilEnumMap *enumMap = (JavaUtilEnumMap *) check_class_cast(object, [JavaUtilEnumMap class]);
...
- (id)getWithId:(id)key {
  ...
  int keyOrdinal = [((JavaLangEnum *) nil_chk(key)) ordinal];
  return (id) IOSObjectArray_Get(nil_chk(values__), keyOrdinal);
}
...
- (id)removeWithId:(id)key {
  ...
  int keyOrdinal = [((JavaLangEnum *) nil_chk(key)) ordinal];
  if (IOSBooleanArray_Get(nil_chk(hasMapping_), keyOrdinal)) {
    (*IOSBooleanArray_GetRef(hasMapping_, keyOrdinal)) = NO;
    mappingsCount_--;
  }
  id oldValue = (id) IOSObjectArray_Get(nil_chk(values__), keyOrdinal);
  IOSObjectArray_Set(values__, keyOrdinal, nil);
  return oldValue;
}
...
- (void)putAllImplWithJavaUtilMap:(id<JavaUtilMap>)map {
  ...
  while ([((id<JavaUtilIterator>) nil_chk(iter)) hasNext]) {
    id<JavaUtilMap_Entry> entry = (id<JavaUtilMap_Entry>) check_protocol_cast([iter next], @protocol(JavaUtilMap_Entry));
...
- (id)putImplWithId:(JavaLangEnum *)key
  ...
  int keyOrdinal = [((JavaLangEnum *) nil_chk(key)) ordinal];
  if (!IOSBooleanArray_Get(nil_chk(hasMapping_), keyOrdinal)) {
    (*IOSBooleanArray_GetRef(hasMapping_, keyOrdinal)) = YES;
    mappingsCount_++;
  }
  id oldValue = (id) IOSObjectArray_Get(nil_chk(values__), keyOrdinal);
  IOSObjectArray_Set(values__, keyOrdinal, value);
  return oldValue;
}
...
- (BOOL)isEqual:(id)object {
  if (!IOSBooleanArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->hasMapping_), ordinal_)) {
    return NO;
  }
  BOOL isEqual = NO;
  if ([object conformsToProtocol: @protocol(JavaUtilMap_Entry)]) {
    ...
    if ([((JavaLangEnum *) nil_chk(key_)) isEqual:enumKey]) {
      ...
      isEqual = IOSObjectArray_Get(nil_chk(enumMap_->values__), ordinal_) == nil ? nil == theValue : [nil_chk(IOSObjectArray_Get(enumMap_->values__, ordinal_)) isEqual:theValue];
    }
  }
  return isEqual;
}
...
- (NSUInteger)hash {
  return (IOSObjectArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->keys_), ordinal_) == nil ? 0 : [((JavaLangEnum *) nil_chk(IOSObjectArray_Get(enumMap_->keys_, ordinal_))) hash]) ^ (IOSObjectArray_Get(nil_chk(enumMap_->values__), ordinal_) == nil ? 0 : [nil_chk(IOSObjectArray_Get(enumMap_->values__, ordinal_)) hash]);
}

- (id)getKey {
  [self checkEntryStatus];
  return (JavaLangEnum *) IOSObjectArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->keys_), ordinal_);
}

- (id)getValue {
  [self checkEntryStatus];
  return (id) IOSObjectArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->values__), ordinal_);
}

- (id)setValueWithId:(id)value {
  [self checkEntryStatus];
  return [enumMap_ putWithId:(JavaLangEnum *) IOSObjectArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->keys_), ordinal_) withId:value];
}

- (NSString *)description {
  JavaLangStringBuilder *result = [[[JavaLangStringBuilder alloc] initWithNSString:[((JavaLangEnum *) nil_chk(IOSObjectArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->keys_), ordinal_))) description]] autorelease];
  [result appendWithNSString:@"="];
  [result appendWithNSString:[nil_chk(IOSObjectArray_Get(nil_chk(enumMap_->values__), ordinal_)) description]];
...
- (BOOL)removeWithId:(id)object {
  if (nil == object) {
    for (int i = 0; i < ((JavaUtilEnumMap *) nil_chk(enumMap_))->enumSize_; i++) {
      if (IOSBooleanArray_Get(nil_chk(enumMap_->hasMapping_), i) && nil == IOSObjectArray_Get(nil_chk(enumMap_->values__), i)) {
        [enumMap_ removeWithId:IOSObjectArray_Get(nil_chk(enumMap_->keys_), i)];
        return YES;
      }
    }
  }
    ...
    for (int i = 0; i < ((JavaUtilEnumMap *) nil_chk(enumMap_))->enumSize_; i++) {
      if (IOSBooleanArray_Get(nil_chk(enumMap_->hasMapping_), i) && [object isEqual:IOSObjectArray_Get(nil_chk(enumMap_->values__), i)]) {
        [enumMap_ removeWithId:IOSObjectArray_Get(nil_chk(enumMap_->keys_), i)];
        return YES;
      }
    }
...
- (BOOL)hasNext {
  ...
  for (; position_ < length; position_++) {
    if (IOSBooleanArray_Get(nil_chk(enumMap_->hasMapping_), position_)) {
      break;
    }
  }
...
- (id)next {
  ...
  return [((id<JavaUtilMapEntry_Type>) nil_chk(type_)) getWithJavaUtilMapEntry:[[[JavaUtilMapEntry alloc] initWithId:IOSObjectArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->keys_), prePosition_) withId:IOSObjectArray_Get(nil_chk(enumMap_->values__), prePosition_)] autorelease]];
}

- (void)remove {
  [self checkStatus];
  if (IOSBooleanArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->hasMapping_), prePosition_)) {
    [enumMap_ removeWithId:IOSObjectArray_Get(nil_chk(enumMap_->keys_), prePosition_)];
  }
  prePosition_ = -1;
}
...
- (NSString *)description {
  ...
  return [nil_chk([((id<JavaUtilMapEntry_Type>) nil_chk(type_)) getWithJavaUtilMapEntry:[[[JavaUtilMapEntry alloc] initWithId:IOSObjectArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->keys_), prePosition_) withId:IOSObjectArray_Get(nil_chk(enumMap_->values__), prePosition_)] autorelease]]) description];
}
...
- (id)next {
  ...
  prePosition_ = position_++;
  return [((id<JavaUtilMapEntry_Type>) nil_chk(type_)) getWithJavaUtilMapEntry:[[[JavaUtilEnumMap_Entry alloc] initWithId:(JavaLangEnum *) IOSObjectArray_Get(nil_chk(((JavaUtilEnumMap *) nil_chk(enumMap_))->keys_), prePosition_) withId:(id) IOSObjectArray_Get(nil_chk(enumMap_->values__), prePosition_) withJavaUtilEnumMap:enumMap_] autorelease]];
}
