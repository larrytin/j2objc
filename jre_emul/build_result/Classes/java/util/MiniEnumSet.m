...
- (BOOL)addAllWithJavaUtilCollection:(id<JavaUtilCollection>)collection {
  ...
  if ([(id) collection isKindOfClass:[JavaUtilEnumSet class]]) {
    ...
    JavaUtilMiniEnumSet *miniSet = (JavaUtilMiniEnumSet *) check_class_cast(set, [JavaUtilMiniEnumSet class]);
    long long int oldBits = bits_;
    long long int newBits = oldBits | miniSet->bits_;
    bits_ = newBits;
    size__ = [JavaLangLong bitCountWithLong:newBits];
...
- (BOOL)containsWithId:(id)object {
  ...
  JavaLangEnum *element = (JavaLangEnum *) check_class_cast(object, [JavaLangEnum class]);
...
- (BOOL)containsAllWithJavaUtilCollection:(id<JavaUtilCollection>)collection {
  ...
  if ([(id) collection isKindOfClass:[JavaUtilMiniEnumSet class]]) {
    JavaUtilMiniEnumSet *set = (JavaUtilMiniEnumSet *) check_class_cast(collection, [JavaUtilMiniEnumSet class]);
...
- (BOOL)isEqual:(id)object {
  ...
  JavaUtilEnumSet *set = (JavaUtilEnumSet *) check_class_cast(object, [JavaUtilEnumSet class]);
  if (![self isValidTypeWithIOSClass:((JavaUtilEnumSet *) nil_chk(set))->elementClass_]) {
    return size__ == 0 && [set isEmpty];
  }
  return bits_ == ((JavaUtilMiniEnumSet *) check_class_cast(set, [JavaUtilMiniEnumSet class]))->bits_;
}
...
- (void)setRangeWithId:(JavaLangEnum *)start
  ...
  bits_ |= range;
  size__ = [JavaLangLong bitCountWithLong:bits_];
}
...
- (id)next {
  ...
  int ordinal = [JavaLangLong numberOfTrailingZerosWithLong:mask_];
  JavaUtilMiniEnumSet_MiniEnumSetIterator_set_last_(self, IOSObjectArray_Get(nil_chk(this$0_->enums_), ordinal));
  currentBits_ &= ~mask_;
