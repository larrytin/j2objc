...
- (void)clear {
  ...
  for (int i = 0; i < (int) [((IOSObjectArray *) nil_chk(elementData_)) count]; i++) {
    [elementData_ replaceObjectAtIndex:i withObject:nil];
  }
  modCount_++;
}
...
- (BOOL)containsValueWithId:(id)value {
  ...
  for (int i = 1; i < (int) [((IOSObjectArray *) nil_chk(elementData_)) count]; i = i + 2) {
    if ([elementData_ objectAtIndex:i] == value) {
      return YES;
    }
  }
  return NO;
...
- (id)getWithId:(id)key {
  ...
  if ([((IOSObjectArray *) nil_chk(elementData_)) objectAtIndex:index] == key) {
    id result = [elementData_ objectAtIndex:index + 1];
...
- (JavaUtilIdentityHashMap_IdentityHashMapEntry *)getEntryWithInt:(int)index {
  return [[[JavaUtilIdentityHashMap_IdentityHashMapEntry alloc] initWithId:(id) [((IOSObjectArray *) nil_chk(elementData_)) objectAtIndex:index] withId:(id) [elementData_ objectAtIndex:index + 1] withNSObjectArray:elementData_] autorelease];
}
...
+ (int)findIndexWithId:(id)key
  ...
  while (index != last) {
    if ([array objectAtIndex:index] == key || ([array objectAtIndex:index] == nil)) {
      break;
    }
...
- (id)putWithId:(id)key
  ...
  if ([((IOSObjectArray *) nil_chk(elementData_)) objectAtIndex:index] != _key) {
    ...
    [elementData_ replaceObjectAtIndex:index withObject:_key];
    [elementData_ replaceObjectAtIndex:index + 1 withObject:nil];
  }
  id result = [elementData_ objectAtIndex:index + 1];
  [elementData_ replaceObjectAtIndex:index + 1 withObject:_value];
  return [self massageValueWithId:result];
}
...
- (void)rehash {
  ...
  for (int i = 0; i < (int) [elementData_ count]; i = i + 2) {
    id key = [elementData_ objectAtIndex:i];
    if (key != nil) {
      ...
      [newData replaceObjectAtIndex:index + 1 withObject:[elementData_ objectAtIndex:i + 1]];
    }
  }
...
- (id)removeWithId:(id)key {
  ...
  result = [elementData_ objectAtIndex:index + 1];
  int length = (int) [elementData_ count];
  while (YES) {
    next = (next + 2) % length;
    object = [elementData_ objectAtIndex:next];
    if (object == nil) {
      break;
    }
    ...
    if (!hashedOk) {
      [elementData_ replaceObjectAtIndex:index withObject:object];
      [elementData_ replaceObjectAtIndex:index + 1 withObject:[elementData_ objectAtIndex:next + 1]];
      index = next;
    }
  }
  size__--;
...
- (id)setValueWithId:(id)object {
  ...
  if ([((IOSObjectArray *) nil_chk(elementData_)) objectAtIndex:index] == key_) {
    [elementData_ replaceObjectAtIndex:index + 1 withObject:object];
  }
...
- (BOOL)hasNext {
  while (position_ < (int) [((IOSObjectArray *) nil_chk(((JavaUtilIdentityHashMap *) nil_chk(associatedMap_))->elementData_)) count]) {
    if ([associatedMap_->elementData_ objectAtIndex:position_] == nil) {
      position_ += 2;
    }
    else {
      return YES;
...
- (void)remove {
  ...
  canRemove_ = NO;
  [associatedMap_ removeWithId:[((IOSObjectArray *) nil_chk(((JavaUtilIdentityHashMap *) nil_chk(associatedMap_))->elementData_)) objectAtIndex:lastPosition_]];
