...
+ (JavaUtilTimeZone *)getCustomTimeZoneWithNSString:(NSString *)id_ {
  ...
  NSString *fmt = [m groupWithInt:2] != nil && ![JavaLangCharacter isDigitWithUnichar:[((NSString *) nil_chk([m groupWithInt:2])) charAtWithInt:0]] ? @"GMT%c%02d%02d" : @"GMT%c%02d:%02d";
...
+ (void)setDefaultWithJavaUtilTimeZone:(JavaUtilTimeZone *)timeZone {
  @synchronized([self class]) {
    {
      JreOperatorRetainedAssign(&JavaUtilTimeZone_defaultTimeZone_, nil, timeZone != nil ? (JavaUtilTimeZone *) [timeZone clone] : nil);
    }
  }
}
