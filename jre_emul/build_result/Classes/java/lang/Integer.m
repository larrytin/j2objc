...
+ (int)parseWithNSString:(NSString *)string
  ...
  while (offset < length) {
    int digit = [JavaLangCharacter digitWithUnichar:[string charAtWithInt:offset++] withInt:radix];
    if (digit == -1) {
      @throw [[[JavaLangNumberFormatException alloc] initWithNSString:string] autorelease];
    }
    if (max > result) {
...
+ (IOSObjectArray *)__exceptions_parseWithNSString_withInt_withInt_withBOOL_ {
  return [IOSObjectArray arrayWithObjects:(id[]) { [JavaLangNumberFormatException getClass] } count:1 type:[IOSClass getClass]];
}
...
+ (NSString *)toBinaryStringWithInt:(int)i {
  ...
  do {
    (*[buffer charRefAtIndex:--count]) = (unichar) ((i & 1) + '0');
...
+ (NSString *)toHexStringWithInt:(int)i {
  ...
  do {
    ...
    (*[buffer charRefAtIndex:--count]) = (unichar) t;
...
+ (NSString *)toOctalStringWithInt:(int)i {
  ...
  IOSCharArray *buffer = [IOSCharArray arrayWithLength:count];
  do {
    (*[buffer charRefAtIndex:--count]) = (unichar) ((i & 7) + '0');
...
+ (NSString *)toStringWithInt:(int)i
  ...
  do {
    ...
    (*[buffer charRefAtIndex:--count]) = (unichar) ch;
  }
  while ((j /= radix) != 0);
  if (negative) {
    (*[buffer charRefAtIndex:0]) = '-';
  }
  return [NSString stringWithOffset:0 length:(int) [buffer count] characters:buffer];
}
...
+ (JavaLangInteger *)valueOfWithInt:(int)i {
  ...
  return [((IOSObjectArray *) nil_chk([JavaLangInteger_valueOfCache CACHE])) objectAtIndex:i + 128];
}

+ (void)initialize {
  if (self == [JavaLangInteger class]) {
    JreOperatorRetainedAssign(&JavaLangInteger_TYPE_, nil, (IOSClass *) [[[IOSIntArray arrayWithLength:0] getClass] getComponentType]);
  }
}
  ...
  if (self == [JavaLangInteger_valueOfCache class]) {
    JreOperatorRetainedAssign(&JavaLangInteger_valueOfCache_CACHE_, nil, [IOSObjectArray arrayWithLength:256 type:[IOSClass classWithClass:[JavaLangInteger class]]]);
    {
      for (int i = -128; i <= 127; i++) {
        [JavaLangInteger_valueOfCache_CACHE_ replaceObjectAtIndex:i + 128 withObject:[[[JavaLangInteger alloc] initWithInt:i] autorelease]];
      }
    }
  }
}
