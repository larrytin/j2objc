...
+ (NSString *)toBinaryStringWithLongInt:(long long int)l {
  ...
  do {
    (*[buffer charRefAtIndex:--count]) = (unichar) ((l & 1) + '0');
    l >>= 1;
  }
  while (count > 0);
...
+ (NSString *)toOctalStringWithLongInt:(long long int)l {
  ...
  IOSCharArray *buffer = [IOSCharArray arrayWithLength:count];
  do {
    (*[buffer charRefAtIndex:--count]) = (unichar) ((l & 7) + '0');
...
+ (JavaLangLong *)valueOfWithLongInt:(long long int)lng {
  ...
  return [((IOSObjectArray *) nil_chk([JavaLangLong_valueOfCache CACHE])) objectAtIndex:128 + (int) lng];
}

+ (void)initialize {
  if (self == [JavaLangLong class]) {
    JreOperatorRetainedAssign(&JavaLangLong_TYPE_, nil, (IOSClass *) [[[IOSLongArray arrayWithLength:0] getClass] getComponentType]);
  }
}
  ...
  if (self == [JavaLangLong_valueOfCache class]) {
    JreOperatorRetainedAssign(&JavaLangLong_valueOfCache_CACHE_, nil, [IOSObjectArray arrayWithLength:256 type:[IOSClass classWithClass:[JavaLangLong class]]]);
    {
      for (int i = -128; i <= 127; i++) {
        [JavaLangLong_valueOfCache_CACHE_ replaceObjectAtIndex:i + 128 withObject:[[[JavaLangLong alloc] initWithLongInt:i] autorelease]];
      }
    }
  }
}
