...
//  source: /Users/retechretech/dev/tools/lib/j2objc/jre_emul/apache_harmony/classlib/modules/luni/src/main/java/java/lang/StrictMath.java
//
...
+ (NSArray *)memDebugStaticReferences {
  ...
  [result addObject:[JreMemDebugStrongReference strongReferenceWithObject:JavaLangStrictMath_random__ name:@"JavaLangStrictMath_random__"]];
  return result;
}

static JavaUtilRandom * JavaLangStrictMath_random__;

+ (double)E {
  return JavaLangStrictMath_E;
}

+ (double)PI {
  return JavaLangStrictMath_PI;
}

+ (JavaUtilRandom *)random_ {
  return JavaLangStrictMath_random__;
}

+ (void)setRandom_WithJavaUtilRandom:(JavaUtilRandom *)random_ {
  JreOperatorRetainedAssign(&JavaLangStrictMath_random__, random_);
}
...
+ (double)random {
  if (JavaLangStrictMath_random__ == nil) JreOperatorRetainedAssign(&JavaLangStrictMath_random__, [[[JavaUtilRandom alloc] init] autorelease]);
  return [((JavaUtilRandom *) NIL_CHK(JavaLangStrictMath_random__)) nextDouble];
}
...
+ (double)toDegreesWithDouble:(double)angrad {
  return angrad * 180.0 / JavaLangStrictMath_PI;
}

+ (double)toRadiansWithDouble:(double)angdeg {
  return angdeg / 180.0 * JavaLangStrictMath_PI;
}
...
+ (double)ulpWithDouble:(double)d {
  if ([JavaLangDouble isInfiniteWithDouble:d]) {
    ...
    return [JavaLangStrictMath powWithDouble:2 withDouble:971];
  }
  d = [JavaLangMath absWithDouble:d];
  return [JavaLangStrictMath nextafterWithDouble:d withDouble:JavaLangDouble_MAX_VALUE] - d;
}
...
+ (float)ulpWithFloat:(float)f {
  if ([JavaLangFloat isNaNWithFloat:f]) {
    ...
    return (float) [JavaLangStrictMath powWithDouble:2 withDouble:104];
  }
  f = [JavaLangMath absWithFloat:f];
  return [JavaLangStrictMath nextafterfWithFloat:f withFloat:JavaLangFloat_MAX_VALUE] - f;
}
