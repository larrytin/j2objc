...
- (int)activeCount {
  ...
  @synchronized (self->childrenGroupsLock_) {
    for (int i = 0; i < numGroups_; i++) {
      count += [((JavaLangThreadGroup *) nil_chk([((IOSObjectArray *) nil_chk(self->childrenGroups_)) objectAtIndex:i])) activeCount];
    }
  }
  return count;
}
...
- (int)activeGroupCount {
  ...
  @synchronized (self->childrenGroupsLock_) {
    for (int i = 0; i < numGroups_; i++) {
      count += 1 + [((JavaLangThreadGroup *) nil_chk([((IOSObjectArray *) nil_chk(self->childrenGroups_)) objectAtIndex:i])) activeGroupCount];
    }
  }
  return count;
}
  ...
  @synchronized (self->childrenThreadsLock_) {
    if (!isDestroyed__) {
      if ((int) [((IOSObjectArray *) nil_chk(childrenThreads_)) count] == numThreads_) {
        ...
        [newThreads replaceObjectAtIndex:numThreads_++ withObject:thread];
        JavaLangThreadGroup_set_childrenThreads_(self, newThreads);
      }
      else {
        [childrenThreads_ replaceObjectAtIndex:numThreads_++ withObject:thread];
      }
    }
    else {
  ...
  @synchronized (self->childrenGroupsLock_) {
    if (!isDestroyed__) {
      if ((int) [((IOSObjectArray *) nil_chk(childrenGroups_)) count] == numGroups_) {
        ...
        [newGroups replaceObjectAtIndex:numGroups_++ withObject:g];
        JavaLangThreadGroup_set_childrenGroups_(self, newGroups);
      }
      else {
        [childrenGroups_ replaceObjectAtIndex:numGroups_++ withObject:g];
      }
    }
    else {
      @throw [[[JavaLangIllegalThreadStateException alloc] init] autorelease];
  ...
  @synchronized (self->childrenThreadsLock_) {
    @synchronized (self->childrenGroupsLock_) {
      ...
      for (int i = 0; i < toDestroy; i++) {
        [((JavaLangThreadGroup *) nil_chk([((IOSObjectArray *) nil_chk(self->childrenGroups_)) objectAtIndex:0])) destroy];
      }
      if (parent_ != nil) {
...
- (int)enumerateGenericWithNSObjectArray:(IOSObjectArray *)enumeration
  ...
  @synchronized (syncLock) {
    for (int i = enumeratingThreads ? numThreads_ : numGroups_; --i >= 0; ) {
      if (!enumeratingThreads || [((JavaLangThread *) nil_chk([immediateCollection objectAtIndex:i])) isAlive]) {
        if (enumerationIndex >= (int) [((IOSObjectArray *) nil_chk(enumeration)) count]) {
          return enumerationIndex;
        }
        [enumeration replaceObjectAtIndex:enumerationIndex++ withObject:[immediateCollection objectAtIndex:i]];
      }
    }
  }
  if (recurse) {
    @synchronized (self->childrenGroupsLock_) {
      for (int i = 0; i < numGroups_; i++) {
        ...
        enumerationIndex = [((JavaLangThreadGroup *) nil_chk([((IOSObjectArray *) nil_chk(childrenGroups_)) objectAtIndex:i])) enumerateGenericWithNSObjectArray:enumeration withBOOL:recurse withInt:enumerationIndex withBOOL:enumeratingThreads];
      }
    }
  }
  return enumerationIndex;
...
- (void)listWithInt:(int)levels {
  ...
  @synchronized (self->childrenThreadsLock_) {
    for (int i = 0; i < numThreads_; i++) {
      ...
      [[JavaLangSystem out] printlnWithId:[((IOSObjectArray *) nil_chk(self->childrenThreads_)) objectAtIndex:i]];
    }
  }
  @synchronized (self->childrenGroupsLock_) {
    for (int i = 0; i < numGroups_; i++) {
      [((JavaLangThreadGroup *) nil_chk([((IOSObjectArray *) nil_chk(self->childrenGroups_)) objectAtIndex:i])) listWithInt:levels + 1];
    }
  }
}
...
- (void)removeWithJavaLangThread:(JavaLangThread *)thread {
  @synchronized (self->childrenThreadsLock_) {
    for (int i = 0; i < numThreads_; i++) {
      if ([((JavaLangThread *) nil_chk([((IOSObjectArray *) nil_chk(childrenThreads_)) objectAtIndex:i])) isEqual:thread]) {
        numThreads_--;
        [JavaLangSystem arraycopyWithId:childrenThreads_ withInt:i + 1 withId:childrenThreads_ withInt:i withInt:numThreads_ - i];
        [childrenThreads_ replaceObjectAtIndex:numThreads_ withObject:nil];
        break;
      }
    }
  }
...
- (void)removeWithJavaLangThreadGroup:(JavaLangThreadGroup *)g {
  @synchronized (self->childrenGroupsLock_) {
    for (int i = 0; i < numGroups_; i++) {
      if ([((JavaLangThreadGroup *) nil_chk([((IOSObjectArray *) nil_chk(childrenGroups_)) objectAtIndex:i])) isEqual:g]) {
        numGroups_--;
        [JavaLangSystem arraycopyWithId:childrenGroups_ withInt:i + 1 withId:childrenGroups_ withInt:i withInt:numGroups_ - i];
        [childrenGroups_ replaceObjectAtIndex:numGroups_ withObject:nil];
        break;
      }
    }
  }
...
- (void)setMaxPriorityWithInt:(int)newMax {
  ...
  if (newMax <= self->maxPriority_) {
    ...
    @synchronized (self->childrenGroupsLock_) {
      for (int i = 0; i < numGroups_; i++) {
        [((JavaLangThreadGroup *) nil_chk([((IOSObjectArray *) nil_chk(self->childrenGroups_)) objectAtIndex:i])) setMaxPriorityWithInt:newMax];
      }
    }
  }
}
