...
+ (NSString *)relativePathWithNSString:(NSString *)base
  ...
  if ([((NSString *) nil_chk(path)) hasPrefix:@"/"]) {
    return [LibcoreNetUrlUrlUtils canonicalizePathWithNSString:path withBOOL:YES];
  }
  else if (base != nil) {
    NSString *combined = [NSString stringWithFormat:@"%@%@", [base substring:0 endIndex:[base lastIndexOf:'/'] + 1], path];
    return [LibcoreNetUrlUrlUtils canonicalizePathWithNSString:combined withBOOL:YES];
  }
  else {
    return path;
  }
...
- (NSString *)toExternalFormWithJavaNetURL:(JavaNetURL *)url {
  return [self toExternalFormWithJavaNetURL:url withBOOL:NO];
}

- (NSString *)toExternalFormWithJavaNetURL:(JavaNetURL *)url
                                  withBOOL:(BOOL)escapeIllegalCharacters {
