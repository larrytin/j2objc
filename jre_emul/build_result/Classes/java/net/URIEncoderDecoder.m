...
+ (void)validateWithNSString:(NSString *)s
  ...
  for (int i = 0; i < [((NSString *) nil_chk(s)) length]; ) {
    ...
    if (!((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || [((NSString *) nil_chk(legal)) indexOf:ch] > -1 || (ch > 127 && ![JavaLangCharacter isSpaceCharWithUnichar:ch] && ![JavaLangCharacter isISOControlWithUnichar:ch]))) {
...
+ (NSString *)quoteIllegalWithNSString:(NSString *)s
  ...
  for (int i = 0; i < [((NSString *) nil_chk(s)) length]; i++) {
    ...
    if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || [((NSString *) nil_chk(legal)) indexOf:ch] > -1 || (ch > 127 && ![JavaLangCharacter isSpaceCharWithUnichar:ch] && ![JavaLangCharacter isISOControlWithUnichar:ch])) {
      ...
      IOSByteArray *bytes = [[NSString stringWithCharacters:[IOSCharArray arrayWithCharacters:(unichar[]){ ch } count:1]] getBytesWithCharset:JavaNetURIEncoderDecoder_encoding_];
      for (int j = 0; j < (int) [((IOSByteArray *) nil_chk(bytes)) count]; j++) {
        [buf appendWithUnichar:'%'];
        [buf appendWithUnichar:[((NSString *) nil_chk(JavaNetURIEncoderDecoder_digits_)) charAtWithInt:([bytes byteAtIndex:j] & (int) 0xf0) >> 4]];
        [buf appendWithUnichar:[JavaNetURIEncoderDecoder_digits_ charAtWithInt:[bytes byteAtIndex:j] & (int) 0xf]];
      }
    }
  }
