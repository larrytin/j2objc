...
- (id)initWithNSString:(NSString *)scheme
  ...
  if ((self = [super init])) {
    ...
    [((JavaNetURI_Helper *) [[[JavaNetURI_Helper alloc] initWithJavaNetURI:self] autorelease]) parseURIWithNSString:[uri description] withBOOL:NO];
    JreMemDebugAdd(self);
  }
  return self;
}
...
- (id)initJavaNetURIWithNSString:(NSString *)scheme
  ...
  if ((self = [super init])) {
    ...
    [((JavaNetURI_Helper *) [[[JavaNetURI_Helper alloc] initWithJavaNetURI:self] autorelease]) parseURIWithNSString:[uri description] withBOOL:YES];
    JreMemDebugAdd(self);
  }
  return self;
}
...
- (NSString *)normalizeWithNSString:(NSString *)path {
  ...
  while ((index2 = [path indexOf:'/' fromIndex:index + 1]) != -1) {
    [seglist replaceObjectAtIndex:current++ withObject:[path substring:index endIndex:index2]];
    index = index2 + 1;
  }
  if (current < size) {
    [seglist replaceObjectAtIndex:current withObject:[path substring:index]];
  }
  for (int i = 0; i < size; i++) {
    (*[include booleanRefAtIndex:i]) = YES;
    if ([((NSString *) nil_chk([seglist objectAtIndex:i])) isEqual:@".."]) {
      int remove = i - 1;
      while (remove > -1 && ![include booleanAtIndex:remove]) {
        remove--;
      }
      if (remove > -1 && ![((NSString *) nil_chk([seglist objectAtIndex:remove])) isEqual:@".."]) {
        (*[include booleanRefAtIndex:remove]) = NO;
        (*[include booleanRefAtIndex:i]) = NO;
      }
    }
    else if ([((NSString *) nil_chk([seglist objectAtIndex:i])) isEqual:@"."]) {
      (*[include booleanRefAtIndex:i]) = NO;
    }
  }
  JavaLangStringBuilder *newpath = [[[JavaLangStringBuilder alloc] init] autorelease];
  if ([path hasPrefix:@"/"]) {
    [newpath appendWithUnichar:'/'];
  }
  for (int i = 0; i < (int) [seglist count]; i++) {
    if ([include booleanAtIndex:i]) {
      [newpath appendWithNSString:[seglist objectAtIndex:i]];
      [newpath appendWithUnichar:'/'];
    }
  }
  if (![path hasSuffix:@"/"] && (int) [seglist count] > 0 && [include booleanAtIndex:(int) [seglist count] - 1]) {
...
+ (IOSObjectArray *)__exceptions_parseURIWithNSString_withBOOL_ {
  return [IOSObjectArray arrayWithObjects:(id[]) { [JavaNetURISyntaxException getClass] } count:1 type:[IOSClass getClass]];
}
...
+ (IOSObjectArray *)__exceptions_parseAuthorityWithBOOL_ {
  return [IOSObjectArray arrayWithObjects:(id[]) { [JavaNetURISyntaxException getClass] } count:1 type:[IOSClass getClass]];
}
...
- (BOOL)isValidHostWithBOOL:(BOOL)forceServer
  ...
  if (index < 0 || index == [host length] - 1 || ![JavaLangCharacter isDigitWithUnichar:[host charAtWithInt:index + 1]]) {
...
+ (IOSObjectArray *)__exceptions_isValidHostWithBOOL_withNSString_ {
  return [IOSObjectArray arrayWithObjects:(id[]) { [JavaNetURISyntaxException getClass] } count:1 type:[IOSClass getClass]];
}
