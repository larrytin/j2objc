...
+ (BOOL)contentEqualsWithNSString:(NSString *)s
  ...
  for (int i = 0; i < length; i++) {
    if ([((IOSCharArray *) nil_chk(chars)) charAtIndex:start + i] != [s charAtWithInt:i]) {
      return NO;
    }
  }
  return YES;
...
- (NSString *)getWithCharArray:(IOSCharArray *)array
  ...
  NSString *pooled = [pool_ objectAtIndex:index];
  ...
  [pool_ replaceObjectAtIndex:index withObject:result];
  return result;
}
