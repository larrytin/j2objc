...
+ (IOSByteArray *)getBytesWithNSString:(NSString *)name {
  if (OrgApacheHarmonyLuniUtilUtil_defaultEncoding_ != nil) {
    @try {
      return [((NSString *) nil_chk(name)) getBytesWithCharsetName:OrgApacheHarmonyLuniUtilUtil_defaultEncoding_];
    }
...
+ (IOSByteArray *)getUTF8BytesWithNSString:(NSString *)name {
  @try {
    return [((NSString *) nil_chk(name)) getBytesWithCharsetName:@"UTF-8"];
  }
  @catch (JavaIoUnsupportedEncodingException *e) {
...
+ (long long int)parseDateWithNSString:(NSString *)string {
  ...
  while (offset <= length) {
    ...
    if (nextState == LETTERS || nextState == NUMBERS) [buffer appendWithChar:next];
    state = nextState;
  }
...
+ (int)parseWithNSString:(NSString *)string
  ...
  for (int i = 0; i < (int) [((IOSObjectArray *) nil_chk(array)) count]; i++) {
    if ([string regionMatches:YES thisOffset:0 aString:IOSObjectArray_Get(array, i) otherOffset:0 count:length]) return i;
  }
  return -1;
}
...
+ (NSString *)convertUTF8WithBufWithByteArray:(IOSByteArray *)buf
  ...
  int count = 0, s = 0, a;
  while (count < utfSize) {
    if (((*IOSCharArray_GetRef(nil_chk(outArg), s)) = (unichar) IOSByteArray_Get(nil_chk(buf), offset + count++)) < 0x0080) s++;
    else if (((a = IOSCharArray_Get(outArg, s)) & (int) 0xe0) == (int) 0xc0) {
      ...
      int b = IOSByteArray_Get(buf, count++);
      ...
      (*IOSCharArray_GetRef(outArg, s++)) = (unichar) (((a & (int) 0x1F) << 6) | (b & (int) 0x3F));
    }
      ...
      int b = IOSByteArray_Get(buf, count++);
      int c = IOSByteArray_Get(buf, count++);
      ...
      (*IOSCharArray_GetRef(outArg, s++)) = (unichar) (((a & (int) 0x0F) << 12) | ((b & (int) 0x3F) << 6) | (c & (int) 0x3F));
    }
    else {
...
+ (NSString *)decodeWithNSString:(NSString *)s
  ...
  return [OrgApacheHarmonyLuniUtilUtil decodeWithNSString:s withBoolean:convertPlus withNSString:nil];
}
...
+ (NSString *)decodeWithNSString:(NSString *)s
  ...
  for (int i = 0; i < [s length]; ) {
    ...
    if (convertPlus && c == '+') [result appendWithChar:' '];
    else if (c == '%') {
      [out reset];
      do {
...
+ (NSString *)encodeURLWithNSString:(NSString *)urlStr {
  ...
  for (int i = 0; i < [urlStr length]; i++) {
    ...
    if ('%' == ch) {
      if (i + 1 < [urlStr length] && i + 2 < [urlStr length] && [digits indexOf:[JavaLangCharacter toUpperCaseWithChar:[urlStr charAtWithInt:i + 1]]] != -1 && [digits indexOf:[JavaLangCharacter toUpperCaseWithChar:[urlStr charAtWithInt:i + 2]]] != -1) {
        [buf appendWithChar:ch];
        [buf appendWithChar:[urlStr charAtWithInt:i + 1]];
        [buf appendWithChar:[urlStr charAtWithInt:i + 2]];
        i += 2;
      }
      else {
      ...
      @try {
        bytes = [[NSString stringWithCharacters:[IOSCharArray arrayWithCharacters:(unichar[]){ ch } count:1]] getBytesWithCharsetName:@"UTF-8"];
      }
      @catch (JavaIoUnsupportedEncodingException *e) {
      ...
      for (int j = 0; j < (int) [((IOSByteArray *) nil_chk(bytes)) count]; j++) {
        [buf appendWithChar:'%'];
        [buf appendWithChar:[digits charAtWithInt:(IOSByteArray_Get(bytes, j) & (int) 0xf0) >> 4]];
        [buf appendWithChar:[digits charAtWithInt:IOSByteArray_Get(bytes, j) & (int) 0xf]];
      }
    }
  }
...
+ (NSString *)toASCIILowerCaseWithNSString:(NSString *)s {
  ...
  for (int i = 0; i < len; i++) {
    unichar c = [s charAtWithInt:i];
    if ('A' <= c && c <= 'Z') {
      [buffer appendWithChar:(unichar) (c + ('a' - 'A'))];
    }
    else {
...
+ (void)initialize {
  if (self == [OrgApacheHarmonyLuniUtilUtil class]) {
    ...
    JreOperatorRetainedAssign(&OrgApacheHarmonyLuniUtilUtil_MONTHS_, nil, [IOSObjectArray arrayWithObjects:(id[]){ @"January", @"February", @"March", @"April", @"May", @"June", @"July", @"August", @"September", @"October", @"November", @"December" } count:12 type:[IOSClass classWithClass:[NSString class]]]);
    {
      ...
      if (encoding != nil) {
        @try {
          [@"" getBytesWithCharsetName:encoding];
        }
