...
- (void)startElementWithNSString:(NSString *)qName
  ...
  for (int i = 0; i < length; i++) {
    ...
    @try {
      ...
      [atts_ addAttributeWithNSString:[((IOSObjectArray *) nil_chk(attName)) objectAtIndex:0] withNSString:[attName objectAtIndex:1] withNSString:[attName objectAtIndex:2] withNSString:type withNSString:value];
    }
  ...
  if (exceptions != nil && errorHandler_ != nil) {
    {
      id<JavaUtilIterator> iter__ = [exceptions iterator];
      while ([((id<JavaUtilIterator>) nil_chk(iter__)) hasNext]) {
        OrgXmlSaxSAXParseException *ex = [iter__ next];
  ...
  if (contentHandler_ != nil) {
    ...
    [contentHandler_ startElementWithNSString:[((IOSObjectArray *) nil_chk(name)) objectAtIndex:0] withNSString:[name objectAtIndex:1] withNSString:[name objectAtIndex:2] withOrgXmlSaxAttributes:atts_];
  }
}
...
- (void)endElementWithNSString:(NSString *)qName {
  ...
  if (contentHandler_ != nil) {
    [contentHandler_ endElementWithNSString:[((IOSObjectArray *) nil_chk(names)) objectAtIndex:0] withNSString:[names objectAtIndex:1] withNSString:[names objectAtIndex:2]];
    ...
    while ([((id<JavaUtilEnumeration>) nil_chk(prefixes)) hasMoreElements]) {
      NSString *prefix = (NSString *) [prefixes nextElement];
...
- (void)setupParser {
  ...
  if (uris_) [nsSupport_ setNamespaceDeclUrisWithBOOL:YES];
...
- (IOSObjectArray *)processNameWithNSString:(NSString *)qName
  ...
  if (parts == nil) {
    ...
    [parts replaceObjectAtIndex:2 withObject:[((NSString *) nil_chk(qName)) intern]];
  }
  return parts;
}

+ (IOSObjectArray *)__exceptions_processNameWithNSString_withBOOL_withBOOL_ {
  return [IOSObjectArray arrayWithObjects:(id[]) { [OrgXmlSaxSAXException getClass] } count:1 type:[IOSClass getClass]];
}
