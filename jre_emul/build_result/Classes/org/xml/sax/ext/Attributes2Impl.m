...
- (BOOL)isDeclaredWithInt:(int)index {
  ...
  return IOSBooleanArray_Get(nil_chk(declared_), index);
}
...
- (BOOL)isSpecifiedWithInt:(int)index {
  if (index < 0 || index >= [self getLength]) @throw [[[JavaLangArrayIndexOutOfBoundsException alloc] initWithNSString:[NSString stringWithFormat:@"No attribute at index: %d", index]] autorelease];
  return IOSBooleanArray_Get(nil_chk(specified_), index);
}
...
- (void)setAttributesWithOrgXmlSaxAttributes:(id<OrgXmlSaxAttributes>)atts {
  ...
  if ([(id) atts conformsToProtocol: @protocol(OrgXmlSaxExtAttributes2)]) {
    id<OrgXmlSaxExtAttributes2> a2 = (id<OrgXmlSaxExtAttributes2>) check_protocol_cast(atts, @protocol(OrgXmlSaxExtAttributes2));
    for (int i = 0; i < length; i++) {
      (*IOSBooleanArray_GetRef(declared_, i)) = [a2 isDeclaredWithInt:i];
      (*IOSBooleanArray_GetRef(specified_, i)) = [a2 isSpecifiedWithInt:i];
    }
  }
  else {
    for (int i = 0; i < length; i++) {
      (*IOSBooleanArray_GetRef(declared_, i)) = ![@"CDATA" isEqual:[atts getTypeWithInt:i]];
      (*IOSBooleanArray_GetRef(specified_, i)) = YES;
    }
  }
}
...
- (void)addAttributeWithNSString:(NSString *)uri
  ...
  (*IOSBooleanArray_GetRef(specified_, length - 1)) = YES;
  (*IOSBooleanArray_GetRef(nil_chk(declared_), length - 1)) = ![@"CDATA" isEqual:type];
}
...
- (void)setDeclaredWithInt:(int)index
  ...
  if (index < 0 || index >= [self getLength]) @throw [[[JavaLangArrayIndexOutOfBoundsException alloc] initWithNSString:[NSString stringWithFormat:@"No attribute at index: %d", index]] autorelease];
  (*IOSBooleanArray_GetRef(nil_chk(declared_), index)) = value;
}

- (void)setSpecifiedWithInt:(int)index
                withBoolean:(BOOL)value {
  if (index < 0 || index >= [self getLength]) @throw [[[JavaLangArrayIndexOutOfBoundsException alloc] initWithNSString:[NSString stringWithFormat:@"No attribute at index: %d", index]] autorelease];
  (*IOSBooleanArray_GetRef(nil_chk(specified_), index)) = value;
}
