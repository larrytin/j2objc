...
- (void)setFeatureWithNSString:(NSString *)name
  ...
  [((JavaUtilHashMap *) nil_chk(features_)) putWithId:name withId:[JavaLangBoolean valueOfWithBoolean:state]];
}
...
- (BOOL)getFeatureWithNSString:(NSString *)name {
  JavaLangBoolean *value = (JavaLangBoolean *) check_class_cast([((JavaUtilHashMap *) nil_chk(features_)) getWithId:name], [JavaLangBoolean class]);
  return value != nil ? [value booleanValue] : NO;
}

- (void)setNamespaceAwareWithBoolean:(BOOL)awareness {
  [((JavaUtilHashMap *) nil_chk(features_)) putWithId:[OrgXmlpullV1XmlPullParser FEATURE_PROCESS_NAMESPACES] withId:[JavaLangBoolean valueOfWithBoolean:awareness]];
}
...
- (void)setValidatingWithBoolean:(BOOL)validating {
  [((JavaUtilHashMap *) nil_chk(features_)) putWithId:[OrgXmlpullV1XmlPullParser FEATURE_VALIDATION] withId:[JavaLangBoolean valueOfWithBoolean:validating]];
}
...
- (id<OrgXmlpullV1XmlPullParser>)newPullParser {
  ...
  for (int i = 0; i < [parserClasses_ size]; i++) {
    IOSClass *ppClass = (IOSClass *) check_class_cast([parserClasses_ getWithInt:i], [IOSClass class]);
    @try {
      id<OrgXmlpullV1XmlPullParser> pp = (id<OrgXmlpullV1XmlPullParser>) check_protocol_cast([((IOSClass *) nil_chk(ppClass)) newInstance], @protocol(OrgXmlpullV1XmlPullParser));
      for (id<JavaUtilIterator> iter = [((id<JavaUtilSet>) nil_chk([((JavaUtilHashMap *) nil_chk(features_)) keySet])) iterator]; [((id<JavaUtilIterator>) nil_chk(iter)) hasNext]; ) {
        NSString *key = (NSString *) check_class_cast([iter next], [NSString class]);
        JavaLangBoolean *value = (JavaLangBoolean *) check_class_cast([features_ getWithId:key], [JavaLangBoolean class]);
        if (value != nil && [value booleanValue]) {
          [((id<OrgXmlpullV1XmlPullParser>) nil_chk(pp)) setFeatureWithNSString:key withBoolean:YES];
        }
      }
      return pp;
    }
...
- (id<OrgXmlpullV1XmlSerializer>)newSerializer {
  ...
  for (int i = 0; i < [serializerClasses_ size]; i++) {
    IOSClass *ppClass = (IOSClass *) check_class_cast([serializerClasses_ getWithInt:i], [IOSClass class]);
    @try {
      id<OrgXmlpullV1XmlSerializer> ser = (id<OrgXmlpullV1XmlSerializer>) check_protocol_cast([((IOSClass *) nil_chk(ppClass)) newInstance], @protocol(OrgXmlpullV1XmlSerializer));
      return ser;
    }
    @catch (JavaLangException *ex) {
      [issues appendWithNSString:[NSString stringWithFormat:@"%@: %@; ", [((IOSClass *) nil_chk(ppClass)) getName], [((JavaLangException *) nil_chk(ex)) description]]];
  ...
  while (pos < [classNames length]) {
    ...
    if (candidate != nil) {
      ...
      if ([instance isKindOfClass:[OrgXmlpullV1XmlPullParserFactory class]]) {
        if (factory == nil) {
          factory = (OrgXmlpullV1XmlPullParserFactory *) check_class_cast(instance, [OrgXmlpullV1XmlPullParserFactory class]);
        }
        recognized = YES;
      }
      if (!recognized) {
