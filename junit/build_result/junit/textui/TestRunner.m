...
+ (void)runAndWaitWithJunitFrameworkTest:(id<JunitFrameworkTest>)suite {
  ...
  [aTestRunner doRunWithJunitFrameworkTest:suite withBOOL:YES];
}
...
- (JunitFrameworkTestResult *)doRunWithJunitFrameworkTest:(id<JunitFrameworkTest>)test {
  return [self doRunWithJunitFrameworkTest:test withBOOL:NO];
}
...
- (JunitFrameworkTestResult *)doRunWithJunitFrameworkTest:(id<JunitFrameworkTest>)suite
  ...
  [((JunitTextuiResultPrinter *) nil_chk(fPrinter_)) printWithJunitFrameworkTestResult:result withLongInt:runTime];
  [self pauseWithBOOL:wait];
  return result;
}
...
- (JunitFrameworkTestResult *)startWithNSStringArray:(IOSObjectArray *)args {
  ...
  for (int i = 0; i < (int) [((IOSObjectArray *) nil_chk(args)) count]; i++) {
    if ([((NSString *) nil_chk([args objectAtIndex:i])) isEqual:@"-wait"]) wait = YES;
    else if ([((NSString *) nil_chk([args objectAtIndex:i])) isEqual:@"-c"]) testCase = [self extractClassNameWithNSString:[args objectAtIndex:++i]];
    else if ([((NSString *) nil_chk([args objectAtIndex:i])) isEqual:@"-m"]) {
      NSString *arg = [args objectAtIndex:++i];
      int lastIndex = [((NSString *) nil_chk(arg)) lastIndexOf:'.'];
      testCase = [arg substring:0 endIndex:lastIndex];
      method = [arg substring:lastIndex + 1];
    }
    else if ([((NSString *) nil_chk([args objectAtIndex:i])) isEqual:@"-v"]) [((JavaIoPrintStream *) nil_chk([JavaLangSystem err])) printlnWithNSString:[NSString stringWithFormat:@"JUnit %@ by Kent Beck and Erich Gamma", [JunitRunnerVersion id__]]];
    else testCase = [args objectAtIndex:i];
  }
  ...
  @try {
    if (![((NSString *) nil_chk(method)) isEqual:@""]) return [self runSingleMethodWithNSString:testCase withNSString:method withBOOL:wait];
    id<JunitFrameworkTest> suite = [self getTestWithNSString:testCase];
    return [self doRunWithJunitFrameworkTest:suite withBOOL:wait];
  }
  @catch (JavaLangException *e) {
...
- (JunitFrameworkTestResult *)runSingleMethodWithNSString:(NSString *)testCase
  ...
  return [self doRunWithJunitFrameworkTest:test withBOOL:wait];
}

+ (IOSObjectArray *)__exceptions_runSingleMethodWithNSString_withNSString_withBOOL_ {
  return [IOSObjectArray arrayWithObjects:(id[]) { [JavaLangException getClass] } count:1 type:[IOSClass getClass]];
}
